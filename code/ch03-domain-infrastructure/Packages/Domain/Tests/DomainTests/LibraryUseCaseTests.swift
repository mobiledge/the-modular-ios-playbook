...
final class LibraryUseCaseTests: XCTestCase {
    ...
    func testTwoDifferentMediaTypesWithTheSameIdAreNotDuplicates() {
        ...
        let podcastEpisode = SavedItem(id: 1, title: "The Daily", mediaType: .podcast, savedAt: Date())

        XCTAssertTrue(useCase.save(track))
        XCTAssertTrue(useCase.save(podcastEpisode))
    ...
    func testListIsSortedNewestFirst() {
        ...
        let newer = SavedItem(id: 2, title: "Newer", mediaType: .podcast, savedAt: Date(timeIntervalSince1970: 1_000))
